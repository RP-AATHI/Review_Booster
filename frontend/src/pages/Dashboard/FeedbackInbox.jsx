...
import { MessageSquare, AlertCircle, Phone, CheckCircle, Search, Clock } from 'lucide-react';
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                ...
                <div className="flex items-center gap-2 bg-white rounded-lg p-1 border border-gray-200">
                    <button
                        ...
                        className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${filter === 'all' ? 'bg-gray-100 text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                        All
                    </button>
                    <button
                        onClick={() => setFilter('unread')}
                        className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${filter === 'unread' ? 'bg-red-50 text-red-700' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                        Unresolved
                    </button>
                    <button
                        onClick={() => setFilter('resolved')}
                        className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${filter === 'resolved' ? 'bg-green-50 text-green-700' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                        Resolved
                    </button>
                </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm flex-1 overflow-hidden flex flex-col">
                {filteredList.length === 0 ? (
                    <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
                        ...
                        <p className="text-gray-500">No {filter !== 'all' ? filter : ''} feedback currently matching your filters.</p>
                    </div>
                ) : (
                    <div className="overflow-y-auto flex-1 p-2">
                        <div className="space-y-3 p-4">
                            {filteredList.map((feedback) => (
                                <div
                                    key={feedback.id}
                                    className={`p-5 rounded-lg border transition-all ${feedback.status === 'unread'
                                ...
                                >
                                    <div className="flex justify-between items-start mb-3">
                                        <div className="flex items-center gap-3">
                                            <div className={`px-3 py-1 text-xs font-bold uppercase tracking-wider rounded-full ${getIssueColor(feedback.issueCategory)}`}>
                                    ...
                                    <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100/50">
                                        ...
                                        {feedback.status === 'unread' ? (
                                            <button
                                                ...
                                                className="text-sm bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 px-4 py-1.5 rounded-md font-medium shadow-sm transition-colors"
                                            >
                                                Mark Resolved
                                            </button>
                                        ) : (
                                            <span className="text-sm text-green-600 flex items-center gap-1 font-medium bg-green-50 px-3 py-1.5 rounded-md">
